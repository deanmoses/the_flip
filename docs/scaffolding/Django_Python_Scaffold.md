...
Day-to-day feature work should reference [`../Django_Python.md`](../Django_Python.md); this file exists so we can recreate the same foundation consistently.
...
- Split configuration into `the_flip/settings/base.py`, `dev.py`, `test.py`, and `prod.py`.
  ...
  - `prod.py` enforces security flags (`SECURE_HSTS_SECONDS`, `SECURE_SSL_REDIRECT`, `CSRF_COOKIE_SECURE`, `SESSION_COOKIE_SECURE`), turns off `DEBUG`, and configures WhiteNoise/static root for Railway.
- Read secrets and Railway-provided environment variables via `django-environ`/`python-decouple`. Required keys include `SECRET_KEY`, `ALLOWED_HOSTS`, `ADMIN_USERNAME`, `ADMIN_PASSWORD`, and the email settings used by notifications.
...
- These commands run via `build.sh` so every deploy to Railway recreates the demo environment. When rebuilding locally, run the same sequence after `migrate`.

## Deployment Hooks

- `railway.toml` references `build.sh` for builds and `gunicorn the_flip.wsgi:application` for runtime. Keep those commands in sync with the project structure.
...
- Railway service variables:
  ...
  - `DEBUG=false`, `ALLOWED_HOSTS=the-flip-production.up.railway.app`, `PYTHON_VERSION=3.13.2`.
  - Custom admin credentials are sourced from `ADMIN_USERNAME`, `ADMIN_PASSWORD`, `ADMIN_EMAIL`.
  - `DATABASE_URL` automatically provided by Railway when PostgreSQL is attached.

## Source Control & Tooling

- Include `build.sh`, `railway.toml`, `nixpacks.toml`, `runtime.txt`, and `requirements.txt` at the repo root so hosting providers pick them up automatically.
